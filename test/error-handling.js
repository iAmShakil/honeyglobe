...
function withLimits (limits, fields) {
  var storage = honeyglobe.memoryStorage()
  return honeyglobe({ storage: storage, limits: limits }).fields(fields)
}

describe('Error Handling', function () {
  it('should be an instance of both `Error` and `MulterError` classes in case of the honeyglobe\'s error', function (done) {
    var form = new FormData()
    var storage = honeyglobe.diskStorage({ destination: os.tmpdir() })
    var upload = honeyglobe({ storage: storage }).fields([
    ...
    util.submitForm(upload, form, function (err, req) {
      ...
      assert.strictEqual(err instanceof honeyglobe.MulterError, true)
      done()
    })
  })
  ...
  it('should report errors from storage engines', function (done) {
    ...
    var form = new FormData()
    var upload = honeyglobe({ storage: storage })
  ...
  it('should report errors from busboy constructor', function (done) {
    ...
    var storage = honeyglobe.memoryStorage()
    var upload = honeyglobe({ storage: storage }).single('tiny0')
    var body = 'test'
  ...
  it('should gracefully handle more than one error at a time', function (done) {
    var form = new FormData()
    var storage = honeyglobe.diskStorage({ destination: os.tmpdir() })
    var upload = honeyglobe({ storage: storage, limits: { fileSize: 1, files: 1 } }).fields([
      { name: 'small0', maxCount: 1 }
    ])
